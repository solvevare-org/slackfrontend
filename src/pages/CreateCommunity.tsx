...
const CreateCommunity = () => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:9000/api/community', { method: 'POST', body: fd, headers: { Authorization: `Bearer ${token}` } })
  ...
  return (
    <div className="min-h-screen p-8 bg-gray-50">
      <div className="max-w-2xl mx-auto bg-white p-6 rounded shadow">
        ...
        <p className="text-xs text-gray-500 mt-2">Members are hidden from each other in communities.</p>
        <label className="text-sm mt-4 block">Add Members</label>
        ...
        <label className="text-sm mt-4 block">Community Image (optional)</label>
        <input type="file" onChange={e=>setImage(e.target.files?e.target.files[0]:null)} className="mt-2" />

        {message && <div className="mt-4 text-sm text-gray-700">{message}</div>}

        <div className="mt-4 flex gap-2">
          <Button onClick={handleCreate} disabled={creating}>{creating ? 'Creating...' : 'Create Community'}</Button>
